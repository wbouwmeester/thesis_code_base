function [element_loc, element_norm, A] = cone_faces_tri_cap(R, H, N_faces, dx, d_edge, z_max)
%CONE_FACES_TRI_CAP Generate a faced conical array with a triangular
%topology with a cap
%   [element_loc, element_norm] = CONE_FACES_TRI_CAP(R, H, N_faces, dx, d_edge, z_max)
...
%   along x, when the face is projected on the x,y plane. The spacing along
%   y between the elements is calculated so that the triangular grid is 
%   parallel to the face edges. d_edge is the edge clearance for the 
%   elements. z_max determines the height of the array and removes all 
%   elements above that z value. On this height z_max an diamond grid array
%   is placed that is normal to the z-axis as a cap.
%
...
% triangular faces
top_angle = atan2(L_ground,2*H_flat);
dy = dx/(2*tan(top_angle));
...
% Calculate area (assuming no cap)
A = N_faces*0.5*H_flat*L_ground;
end
