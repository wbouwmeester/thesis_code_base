%% CST Farfield Plot waveguide
%  This script plots the far field as exported from CST for the waveguide
%
...
filename = 'waveguide';
modes = [{'TE_{10}'} {'TE_{01}'} {'TE_{11}'} {'TM_{11}'} {'TE_{20}'} {'TE_{02}'}];
...
for i = 1:length(modes)
    plot(rad2deg(theta_res(:,1)), E_abs_db(i, :,1), 'DisplayName', [cell2mat(modes(i))]);
end

xlabel('\theta'); ylabel('Normalised |E| [dB]'); title('\phi = 0');
legend; grid on; ylim([-10 0]); xlim([0 90]);

subplot(2,1,2); hold on;
for i = 1:length(modes)
    plot(rad2deg(theta_res(:,91)), E_abs_db(i,:,91), 'DisplayName', [cell2mat(modes(i))]);
end

xlabel('\theta'); ylabel('Normalised |E| [dB]'); title('\phi = 45');
legend; grid on; ylim([-10 0]); xlim([0 90]);

% Plot X-pol
figure; hold on;
for i = 1:length(modes)
    plot(rad2deg(theta_res(:,46)), X_pol_ver(i,:,46), 'DisplayName', [cell2mat(modes(i))]);
end
...
for i = 1:length(modes)
    ...
    title(['Cross Polarisation Isolation [dB] for the ' cell2mat(modes(i)) ' mode']);
